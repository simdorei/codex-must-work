"""Load the strict persisted subset used by passive monitoring."""
...
from scripts.state import CorruptReason, CorruptStateError, JsonValue, runtime_path
...
def runtime_target_from_values(
    ...
    """Validate fresh locked runtime values for one passive target."""
    ...
    critical_value = values.get("critical_after_ms")
    if critical_value is None:
        critical_value = values.get("restart_after_ms")
    if type(critical_value) is not int or critical_value < 1:
        raise CorruptStateError(path, CorruptReason.INVALID_VALUE)
    try:
        thresholds = Thresholds(
            warning=warning_ms / 1000,
            critical=critical_value / 1000,
        )
...
def claim_completion(values: dict[str, JsonValue], event_id: str, path: Path) -> bool:
    """Persist the first completion claim for one task event."""
...
def advance_target_progress_epoch(
    ...
    """Persist one relevant rollout progress event and its new silence anchor."""
    raw_children = values.get("children")
    raw_target = values.get("parent") if target_id is None else None
    ...
    raw_target["silence_started_at"] = observed_at.astimezone(UTC).isoformat()
    if target_id is None:
        values["parent"] = raw_target
