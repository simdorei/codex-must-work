"""Provide deterministic warning and critical-stall transitions."""
...
class MonitorPhase(StrEnum):
    """Lifecycle phases for one monitored target."""
...
class Action(StrEnum):
    """At most one diagnostic action emitted by an evaluation."""
...
class Thresholds:
    """Warning and critical-stall durations in seconds."""

    warning: Seconds
    critical: Seconds

    def __post_init__(self) -> None:
        """Reject nonfinite, nonpositive, or unordered thresholds."""
        finite = isfinite(self.warning) and isfinite(self.critical)
        if not finite or self.warning <= 0 or self.critical <= self.warning:
            raise ValueError
...
class WaitState:
    ...
    def terminal(self) -> bool:
        """Return whether monitoring should stop for this target."""
...
@dataclass(frozen=True, slots=True)
class SilenceState:
    """Detector state for one target generation."""
...
@dataclass(frozen=True, slots=True)
class Evaluation:
    """Next detector state and at most one diagnostic action."""
...
def initial_state(at: MonotonicSeconds) -> SilenceState:
    """Create a healthy detector at target creation."""
    return SilenceState(
...
def set_wait_state(
    ...
    *,
    ...
    """Apply proven waits without opening a new silence epoch on resume."""
    _ = at  # Kept in the API for deterministic callers; resumes do not re-anchor time.
    if waits.terminal:
    ...
    return replace(
        state,
        phase=MonitorPhase.HEALTHY if state.waits.paused else state.phase,
        waits=waits,
    )
...
def evaluate(
    ...
    """Emit warning and critical diagnostics once per silence interval."""
    phase = state.phase
    warning_emitted = state.warning_emitted
    critical_emitted = state.critical_emitted
    action = Action.NONE
    ...
    elif elapsed < thresholds.critical:
        phase = MonitorPhase.SILENT_WARNED
    elif not critical_emitted:
        phase, action, critical_emitted = MonitorPhase.CRITICAL, Action.CRITICAL, True
    return Evaluation(
        replace(
            state,
            phase=phase,
            warning_emitted=warning_emitted,
            critical_emitted=critical_emitted,
        ),
        action,
    )
