...
type GitArguments = tuple[str, ...]
type GitResult = tuple[int, str]


def _replace_comment(data: bytes) -> bytes:
    return data.replace(b"# keep", b"# CHANGED", 1)


def _replace_trust(data: bytes) -> bytes:
    return data.replace(b"sha256:lazy", b"sha256:other", 1)


def _replace_line_ending(data: bytes) -> bytes:
...
def test_source_gate_requires_exact_detached_or_original_clean_checkout(tmp_path: Path) -> None:
    ...
    source.mkdir()
    answers: dict[GitArguments, GitResult] = {
    ...
    def probe(arguments: GitArguments) -> GitResult:
        return answers[arguments]

    support.verify_source(source, _HEAD, _TREE, probe)
...
def test_source_gate_rejects_every_false_positive(
    ...
    source = (tmp_path / "source").resolve()
    source.mkdir()
    valid: dict[GitArguments, GitResult] = {
        ("rev-parse", "HEAD"): (0, _HEAD),
        ("rev-parse", "HEAD^{tree}"): (0, _TREE),
        ("symbolic-ref", "-q", "HEAD"): (0, "refs/heads/main"),
        ("rev-parse", "--git-dir"): (0, ".git"),
...
def test_home_and_auth_preconditions_do_not_read_auth(
    ...
    auth = home / "auth.json"
    _ = auth.write_text("secret", encoding="utf-8")

    def fail_read(_path: Path) -> Never:
        pytest.fail("auth bytes were read")
...
def test_real_home_gate_rejects_temporary_or_reviewer_home(tmp_path: Path, name: str) -> None:
    ...
    home.mkdir()
    with pytest.raises(ledger.SmokeError, match="normal_codex_home_required"):
...
def test_byte_span_ledger_rejects_complementary_byte_changes(
    ...
    with pytest.raises(ledger.SmokeError, match="non_cmw_config_bytes_changed"):
        _ = ledger.verify_config_transition(before, after, cache, _trust())
...
def test_config_ledger_rejects_wrong_owned_transition(tmp_path: Path, mode: str) -> None:
    ...
    with pytest.raises(ledger.SmokeError, match="allowed_config_delta_mismatch"):
        _ = ledger.verify_config_transition(before, after, cache, trust)
...
def test_second_install_no_write_rejects_every_mutation(tmp_path: Path, change: str) -> None:
    ...
    root.mkdir()
    item = root / "state"
    _ = item.write_bytes(b"before")
    before = ledger.snapshot_tree(root)
    if change == "bytes":
        _ = item.write_bytes(b"after!")
    elif change == "metadata":
        _ = item.chmod(0o600 if os.name != "nt" else 0o444)
    elif change == "identity":
        replacement = root / "replacement"
        _ = replacement.write_bytes(b"before")
        _ = replacement.replace(item)
    else:
        _ = (root / "extra").write_bytes(b"new")
...
def test_effective_source_ledger_rejects_source_swap(tmp_path: Path) -> None:
    ...
    _ = source.write_bytes(b'{"hooks":{"UserPromptSubmit":[]}}')
    before = ledger.snapshot_sources((source,))
    replacement = tmp_path / "replacement"
    _ = replacement.write_bytes(source.read_bytes())
...
def test_cli_requires_all_absolute_arguments_and_idempotent_flag(tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit):
        _ = smoke.parse_args(["--codex-home", "relative"])
...
def test_child_commands_use_absolute_runtime_and_child_only_codex_home(tmp_path: Path) -> None:
    ...
    _ = smoke.run_codex(runtime, home, runner, environment)
